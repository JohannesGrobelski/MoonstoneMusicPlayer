...
public class MediaPlayerService extends Service
        ...
        try {
            if (playListModel.getCurrentSongFile().exists()) {
                ...
                if (((LocalBinder) iBinder) != null) {
                    ((LocalBinder) iBinder).boundServiceListener
                            .selectedSong(playListModel.getCurrentSongFile().getPath());
                }
            ...
            if (requestAudioFocus()) {
                if (!isMediaPlayerPrepared) {// state stop
                    ...
                    if (!mediaPlayer.isPlaying())
                        mediaPlayer.start();
                    updatePlaybackState(mediaPlayer.isPlaying() ? PlaybackStateCompat.STATE_PLAYING
    ...
    public void pause() {
        if (mediaPlayer != null && mediaPlayer.isPlaying()) {
            ...
            mediaNotificationManager.update(getMediaDataFromCurrentSong(),
                    mediaSession.getController().getPlaybackState(), mediaSession.getSessionToken());
        }
    }

    public boolean mediaPlayerReady() {
        return (mediaPlayer != null && isPlayingMusic());
    }

    public boolean mediaPlayerNotNull() {
    ...
    private void stopMedia() {
        ...
        if (playListModel.getCurrentSong() != null
                && playListModel.getCurrentSong().getDuration_ms() >= Audiobook.AUDIOBOOK_CUTOFF_MS) {
            PlaytimePersistence.savePlaytime(this, mediaPlayerCurrentDataSourceUri,
    ...
    public void onCreate() {
        ...
        this.mediaSession.setFlags(
                MediaSessionCompat.FLAG_HANDLES_MEDIA_BUTTONS | MediaSessionCompat.FLAG_HANDLES_TRANSPORT_CONTROLS); // Optionally
                                                                                                                     // set
                                                                                                                     // the
                                                                                                                     // flags
                                                                                                                     // for
    ...
    /**
     * wird aufgerufen um den BufferStatus einer Medienresource, die über
     * Netzwerkgestreamt wird anzuzeigen
     */
    @Override
    ...
    public void onCompletion(MediaPlayer mp) {
        if (mediaPlayer != null) {
            ...
            if (((LocalBinder) iBinder) != null) {
                if (DEBUG)
                    Log.d(TAG, "onCompletion: " + (iBinder != null));
    ...
    public boolean onError(MediaPlayer mp, int what, int extra) {
        ...
        if (((LocalBinder) iBinder) != null)
            ((LocalBinder) iBinder).boundServiceListener.onError(what);
        return false;
    }
    ...
    public void onPrepared(MediaPlayer mp) {
        ...
        if (resumePosition != 0) {
            ...
            if (playListModel.getCurrentSong() != null
                    && playListModel.getCurrentSong().getDuration_ms() >= Audiobook.AUDIOBOOK_CUTOFF_MS) {
                int playtime = PlaytimePersistence.getPlaytime(this,
                        Uri.fromFile(playListModel.getCurrentSongFile()).toString());
    ...
    public void onAudioFocusChange(int focusState) {
        ...
        switch (focusState) {
            ...
            case AudioManager.AUDIOFOCUS_LOSS_TRANSIENT:
                // verlieren Audiofokus für kurze,unbestimmte Zeit verloren (z.B. YouTube
        ...
        if (((LocalBinder) iBinder) != null)
            ((LocalBinder) iBinder).boundServiceListener.onAudioFocusChange(focusState);
    }
    ...
    public File getCurrentSongFile() {
        return playListModel.getCurrentSongFile();
    }
    ...
    public void playSong(File songFile) {
        ...
        // "+playListModel.getCurrentSong().getName(),Toast.LENGTH_LONG).show();
    ...
    public void nextSong() {
        ...
        mediaNotificationManager.update(getMediaDataFromCurrentSong(), mediaSession.getController().getPlaybackState(),
    ...
    public boolean toogleShuffleMode() {
        return playListModel.toogleShuffleMode();
    }

    public PlayListModel.REPEATMODE nextRepeatMode() {
        return playListModel.nextRepeatMode();
    }
    ...
    public void setPlayList(List<File> playList) {
        if (DEBUG)
            Log.d(TAG, "startMediaPlayerService init Playlist: " + playList.size());
